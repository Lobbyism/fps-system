...
const system: System<[World, ClientState]> = (world, state) => {
	for (const [hasWeaponId, hasWeaponRecord] of world.queryChanged(HasWeapon)) {
		...
		const weaponModels = [weaponModel.model];
		const viewModel = world.get(hasWeaponId, ViewModel);
		if (viewModel) {
			weaponModels.push(viewModel.model);
		}
		const hasWeapon = hasWeaponRecord.new;
		weaponModels.forEach((model) => {
			const muzzleOriginAttachment = model.FindFirstChild("MuzzleOrigin", true);
